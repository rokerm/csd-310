...
# Query for store locale
SELECT store_id, locale from store;

# Query for whatabook catalogue
SELECT book_id, book_name, author, details from book;

# Views listing not in user's wishlist
SELECT book_id, book_name, author, details
FROM book
WHERE book_id NOT IN (SELECT book_id FROM wishlist WHERE user_id = 1);

# Insert statement to add to user's wishlist
INSERT INTO wishlist(user_id, book_id)
    VALUES(1, 9)
# Removes book from user's wish list
 DELETE FROM wishlist WHERE user_id = 1 AND book_id = 3;